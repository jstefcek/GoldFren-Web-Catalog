...
def get_vyrobce_names(request):
    ...
    """
    kategorie_id = request.GET.get("kategorie_id")
    if not kategorie_id:
        return JsonResponse({"error": "kategorie_id is required"}, status=400)
    
    vyrobce_objects = get_vyrobce_by_kategorie(kategorie_id)
    if vyrobce_objects:
...
def get_vozidlo_filtered_view(request):
    """
    This function returns vozidlo or vozidla filtered by kategorie_id, vyrobce_id, objem, model, rok_vyroby
    """
    kategorie_id = request.GET.get("kategorie_id", None)
    vyrobce_id = request.GET.get("vyrobce_id", None)
    ...
    vozidla_objects = get_vozidlo_filtered(kategorie_id, vyrobce_id, objem, model, rok_vyroby)
    if vozidla_objects:
