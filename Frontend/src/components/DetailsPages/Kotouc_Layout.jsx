...
export default function BrakePadDetail({ category = "", apiUrl = null }) {
  ...
  if (isValidId) {
    trackSortimentItemView({ sortiment_category: "kotouce", sortiment_id: id });
  }

  // State for controlling vehicle compatibility loading
  const [showVehicles, setShowVehicles] = useState(false);
  const [kotoucData, setKotoucData] = useState({});
  ...
  return (
    <div className="p-4 md:p-4 w-full mb-8">
      ...
      <NavigationStrip
        ...
        description="kotouce_title"
      />

      {/* Page title */}
      <h1 className="text-3xl font-bold text-gray-800 mb-4 mt-8">
        {t("kotouc_title")} - {displayData(kotoucData.cislo_dilu)}
      </h1>
          ...
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-gray-700">{displayData(kotoucData.typ)}</p>
              </div>
        ...
        <div className="lg:w-full rounded-lg shadow border border-gray-200 bg-white p-8">
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Braking */}
              <div>
                <h3 className="text-lg font-medium mb-2">Braking</h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.konkurence_branking)}
                </p>
              </div>

              {/* NGBrakes */}
              <div>
                <h3 className="text-lg font-medium mb-2">NGBrakes</h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.konkurence_ngbrakes)}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="lg:w-full rounded-lg shadow border border-gray-200 bg-white p-6">
          <div className="bg-gray-50 p-4 rounded-md">
            <div className="grid grid-cols-1 md:grid-cols-2 md:grid-cols-3 md:grid-cols-4 gap-6">
              ...
              <div>
                <h3 className="text-lg font-medium mb-2">
                  {t("datagrid.outer_diameter")}
                </h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.vnejsi_prumer)}
                </p>
              </div>
              ...
              <div>
                <h3 className="text-lg font-medium mb-2">
                  {t("datagrid.pitch_diameter")}
                </h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.roztecny_prumer)}
                </p>
              </div>
              ...
              <div>
                <h3 className="text-lg font-medium mb-2">
                  {t("datagrid.inside_diameter")}
                </h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.vnitrni_prumer)}
                </p>
              </div>
              ...
              <div>
                <h3 className="text-lg font-medium mb-2">
                  {t("datagrid.thickness")}
                </h3>
                <p className="text-gray-700">
                  {displayData(kotoucData.tloustka)}
                </p>
              </div>
            </div>
          </div>
        ...
        {showVehicles ? (
          <div className="mt-4">
            <DataGrid
              ...
              apiUrl={`${serverUrl}/api/goldfren/internal/kotouce/vozidla?limit=0&kotouc_id=${kotoucData.id}`}
            />
          </div>
        ) : (
